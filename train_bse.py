...
from model_bse import my_model   # <-- B+SE

def count_params_m(m): return sum(p.numel() for p in m.parameters() if p.requires_grad)/1e6
...
def append_csv_row(path,row):
    import os; exists=os.path.exists(path)
    import csv; f=open(path,'a',newline=''); w=csv.DictWriter(f,fieldnames=list(row.keys()))
    if not exists: w.writeheader(); w.writerow(row); f.close(); return
    w.writerow(row); f.close()

# (dataset + training loop identical to train_be.py)
# For brevity and to avoid mistakes, reuse the exact same code from train_be.py,
# changing only the parser description and default run_tag.

# ---- everything below is copy of train_be.py with minor name tweaks ----
...
parser = argparse.ArgumentParser(description='B+SE Ablation (SFCA: FFT→SE) w/ CSV + per-run outputs')
...
parser.add_argument('--run_tag', type=str, default='B+SE_rank')
...
def main():
    ...
    img_train = os.path.join(outdir,'images_train'); img_test = os.path.join(outdir,'images_test')
    ckpt_best = os.path.join(outdir,'best_checkpoint'); ckpt_dir = os.path.join(outdir,'checkpoint', opt.dataset)
