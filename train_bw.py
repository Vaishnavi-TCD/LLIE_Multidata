...
# IMPORTANT: import the width-only ablation model
from model_bw import my_model


# ======= Helpers for ablation logging =======
...
# ======= Dataset (unchanged logic, kept inline for a self-contained file) =======
...
# ======= Argparse =======
parser = argparse.ArgumentParser(description='Low-Light Image Enhancement Training (B+W Ablation)')
...
# NEW (for ablation log)
parser.add_argument('--run_tag', type=str, default='B+W', help='Identifier for this ablation run')
...
def main():
    ...
    # If the user asked for CUDA but there’s no GPU, switch to CPU gracefully
    ...
    # Keep your existing (adaptive) loss weights unchanged for consistency
    weights = {
        'Charbonnier': 3.0,
        'Perceptual': 2.0,
        'MS_SSIM': 1.0,
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for test_iter, batch in enumerate(data_loader_test, 1):
            ...
            # save_img(out[0].detach().cpu(), f'./images_test/{idx[0]}.png')
        ...
        # ====== Save checkpoints (same policy as your baseline) ======
        if avg_psnr > best_psnr or avg_ssim > best_ssim:
            best_psnr = max(best_psnr, avg_psnr)
            best_ssim = max(best_ssim, avg_ssim)
            # best_path = f"best_checkpoint/netG_model_best_mixed_psnr_{best_psnr:.4f}_ssim_{best_ssim:.4f}.pth"
        ...
        # ckpt_path = f"checkpoint/{opt.dataset}/netG_model_mixed_epoch_{epoch}_psnr_{avg_psnr:.4f}.pth"
        ckpt_path = os.path.join(ckpt_dir, f"netG_model_mixed_epoch_{epoch}_psnr_{avg_psnr:.4f}.pth")

        torch.save(net_g, ckpt_path)
        print(f"Checkpoint saved at {ckpt_path}")

        # Keep your adaptive weights logic (unchanged)
        # (Note this can confound *strict* ablations; ok if you want to keep original behavior)
        # You can comment these two blocks out if you want frozen loss weights.
        # if avg_psnr > best_psnr + 0.01:
        #     weights['Charbonnier'] += adjustment_factor
        # else:
        #     weights['Charbonnier'] = max(0.0, weights['Charbonnier'] - adjustment_factor)
        # if avg_ssim > best_ssim + 0.01:
        #     weights['MS_SSIM'] += adjustment_factor
        # else:
        #     weights['MS_SSIM'] = max(0.0, weights['MS_SSIM'] - adjustment_factor)
